...
<div class="container mt-5">
    <div class="card shadow-sm p-10">
        ...
        <div class="card-body">
            ...
            <#--<div class="col-md-3">-->
            <#--<input title="time" id="dateInput" type="text" class="form-control">-->
            <#--</div>-->
            <#--<div class="col-md-1">-->
            <#--<button id="queryBtn" class="btn btn-primary mb-4">查询</button>-->
            <#--</div>-->
            <#--</div>-->
    ...
    <div class="text-right">
        <a href="/manage/roomListPage" class="btn btn-info mt-2">返回</a>
    </div>
</div>
<!-- 新增会议对话框 -->
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div>
                    ...
                    <div class="form-group">
                        <#--<label for="timePickInput">选择发起人</label>-->
                        <button id="leaderSelectButton" class="form-control">点击选择发起人</button>
                    </div>
                    <div class="form-group">
...
<script>
    ...
    leaderSelectButton.addEventListener('click', function () {
        let opts = []
        departments.forEach(department => {
            opts.push({text: department.name, value: department.id})
        })
        bootbox.prompt({
            title: '请选择发起人所在部门',
            inputType: 'select',
            ...
            callback: function (result) {
                let opts = []
                users.forEach(user => {
                    if (user.department.id.toString() === result) {
                        opts.push({text: user.realName, value: user.id})
                    }
                })
                bootbox.prompt({
                    title: '请选择发起人',
                    ...
                    callback: function (result) {
                        for (let opt of opts) {
                            if (opt.value.toString() === result) {
                                leaderSelectButton.setAttribute('lId', result)
                                leaderSelectButton.innerText = opt.text
                            }
                        }
                    }
                })
    ...
    scheduleSubmitBtn.addEventListener('click', function () {
        ...
        let lId = leaderSelectButton.getAttribute('lId')
        ...
        if (null === lId) {
            bootbox.alert('请选择会议发起人')
        }
        userCheckboxes.forEach(userCheckBox => {
            if (userCheckBox.checked) {
                users.push({id: userCheckBox.getAttribute('uId')})
            } else {
                if (userCheckBox.uId === lId) {
                    userMeetings.push({id: lId})
                }
            }
        })
        let meeting = {}
        meeting.name = name
        meeting.leader = {id: lId}
