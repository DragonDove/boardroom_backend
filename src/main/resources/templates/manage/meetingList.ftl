...
<script>
    ...
    let meetings = []
    fetch('/meeting/all').then(response => response.json()).then(json => {
        ...
        meetings.forEach(meeting => {
            ...
            meeting.conf = '<button mId="' + meeting.id + '" class="btn btn-sm btn-danger removeBtn"><i class="icon icon-remove"></i> 删除</button>'
        })
    ...
    meetingTable.on('click', '.removeBtn', function () {
        let that = this
        bootbox.confirm('确定删除吗？', function (result) {
            if (result) {
                fetch('/meeting?id=' + that.getAttribute('mId'), {
                    method: 'DELETE'
